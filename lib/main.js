...
//
// You _must_ pass your `apiKey` string from <http://punchfork.com/api> and
//  also your application name as the `appName` string.
//  (e.g. `var punchfork = require('punchfork')('your-api-key', 'your-app-name');`)
//
  ...
  req.on('response', function(res) {
    ...
    res.on('end', function() {
      ...
      try {
        ...
        err = 1;
        response = { error : { message : "Invalid JSON from punchfork.com" } };
      }
...
module.exports = function(apiKey, appName, https) {
  ...
  return {
    ...
    // # Random Recipe
    randomRecipe: function(cb) {
      get('/random_recipe', {}, cb);
    },

    // # List of Publishers
    listOfPublishers: function(cb) {
      get('/publishers', {}, cb);
    },
    ...
    // # Rate Limit Status
    rateLimitStatus: function(cb) {
      get('/rate_limit_status', {}, cb);
    }
